...
public class GroupSum {
    ...
    static Boolean groupSum(int start, int[] numbers, int target) {
        return (start >= numbers.length ? target == 0 : null);
    }

    static void recursion() {
        System.out.println(groupSum(start + 1, n, target - n[start]) || groupSum(start + 1, n, target) ? true : false);

    }

    public static void main(String[] args) {
        GroupSum.recursion();
        GroupSum.groupSum(0, n, 14);
        GroupSum.groupSum(0, n, 9);
